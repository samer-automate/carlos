...
export default function CreateRoutineButton() {
  ...
  const [form, setForm] = useState({ name: "", description: "", color: COLORS[1], days: [] as number[] });
  ...
  async function submit(e: React.FormEvent) {
    ...
    setLoading(false);
    setOpen(false);
    setForm({ name: "", description: "", color: COLORS[1], days: [] });
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-md">
            <div className="flex items-center justify-between p-6 border-b border-zinc-800">
              <h3 className="text-lg font-semibold text-white">Nueva rutina</h3>
            ...
            <form onSubmit={submit} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Ej: Push day, Pull day, Legs..."
                />
              </div>
              <div>
                  ...
                  {DAY_NAMES.map((name, i) => (
                    <button
                      ...
                      className={`flex-1 py-1.5 rounded text-xs font-medium transition-colors ${
                        form.days.includes(i) ? "bg-emerald-600 text-white" : "bg-zinc-800 text-zinc-400 hover:bg-zinc-700"
                      }`}
                    >
                      {name}
                    </button>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={loading || !form.name.trim()} className="flex-1 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white text-sm font-medium">
